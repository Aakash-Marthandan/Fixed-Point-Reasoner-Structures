...
| **A3** | **DEC-w384 (2.78M, our champion's class), no digit aug** (build §6.2; amendment 2) | X0's loop and regime | 50k | 2–2.5 h + evals | w1 |
| **A4** | DEC-w384 **+ digit aug** (the redundancy control) | A3 | 50k | 2–2.5 h + evals | w1 |
| **A5** | DEC-w384 + FPA + RI (the assembled objectives) | A3 | 50k | 2–2.5 h + evals | w1 |
| **A7 / A8** (amendment 3) | A3 seed 1 (the DEC's own floor) / A6 seed 1 (the parity pair) | A3 / A6 | 50k | 9 h / 15 h + evals | w4 / w5 (32 only; SKIPPED, labeled, on the 16/8) |
...
- **PM-A-14 Learning check on CPU** (before the go): the DEC-tiny and the field cell-tiny under the identical loop for 1,500 steps on a 64-puzzle corpus — the DEC's cross-entropy must fall comparably (a build that compiles but does not learn is the failure this catches). **READ 2026-09-05: DEC-w32 (176k params, no digit aug) CE 1.68 → 0.83 (min 0.77, first exact solves at step 1,300) vs the field cell hid-96 (≈ 450k, digit aug) 2.17 → 0.91 (min 0.84) under the identical loop at lr 3e-4; 3.6 vs 10.7 it/s on CPU (the nine-field activations). PASS.**
- **PM-A-15 (amendment 2, the PI: "why not 3M?") Widths and the evaluation cost.** The core DEC arms move to w384 = 2.78M (our champion's parameter class: a same-count, different-class row against R3/B0 at 3.0M); A6 stays at w512 = X0's class. Re-pricing exposed that the battery scales with the cell's compute (X0's measured 8-min D16 full and 30-min 20k×k128 scan become ≈ 1.4 h and ≈ 5 h at w384 on a 4-chip worker): the wide arms' final/alt fulls run on 50k, the D64 row on 100k, the scan on 5k × k32 — every such row LABELED and read by the analyzer's per-class integrity gate (a wide arm scanned at the X0-class size is a breach); the headline D16 val-selected full stays on all 422,786 for every arm; DECODER-CLASS reads the D64 row's per-puzzle records. Walls (inferred; the preflight re-prices): w384 train ≈ 5.3 h + battery ≈ 3.5 h on 4 chips; X0-class ≈ 1.5 + 3 h; the 16's long pole = A6 ≈ 15 h → deadline cap launch + 18 h, ≈ $205 + weather. The 8-shape fallback is a TWO-NIGHT affair (priority A3 A0 A1 A2 A5 A4; the chain is idempotent across relaunches; the analyzer reads NO-DATA for unfinished arms).
- **PM-A-16 (amendment 3, the PI: the approved v6e-32 in us-east1-d in the weekend US window; budget ≈ $1,200 left for Sudoku).** A 32 does not shorten any arm (a worker = one host's four chips; the parity arm stays the ≈ 15 h pole) — it doubles the workers, so it pays only if the extra workers carry science: A7 = A3 seed 1 (the DEC's OWN floor at w384: the wide-arm contrasts read against it, rule R-A-7, instead of borrowing X0's floor) and A8 = A6 seed 1 (the parity arm's pair, the seeded headline Night B would otherwise buy). 8×4 map: w0 A3 · w1 A4 · w2 A5 · w3 A6 · w4 A7 · w5 A8 · w6 A0 A1 · w7 A2; every worker ≤ 15 h. Cost ≈ 15 h × $27.3 ≈ $410 + weather (band $400–480); the ladder 32 → 16 → 8 keeps the science on a demotion (A7/A8, then A6, become SKIPPED, labeled). Budget after A ≈ $720–800: Night B ≈ $300–400 (the commit head + the syndrome instrument on the carried width; the seeds are already in hand), Night C ≈ $250–350 → the Sudoku programme closes inside ≈ $1,200 with little margin; the ARC-d96 rung draws on the rest of the envelope.
- **PM-A-17 (the PI: switching 32 ↔ 16 ↔ 8 over churn without losing progress; 5-min banking for everything).** The chain restores the live bank at every start and each arm resumes from its banked `ckpt_latest` (the trainer's RESUMED path; the SOT carry restarts, labeled — the sportC1/C2 convention); banked arms (ARM_OK) are skipped on any shape; the static maps for 8/4/1 workers pick up in-flight arms on the new shape. Changes: `--ckpt-every 500` on every arm (≤ 3–5 min between `ckpt_latest` writes at the wide arms' pace, so the 5-min live-bank loop never finds a checkpoint older than ~10 min); shape-specific preflight markers (`PREFLIGHT_OK_w<W>_nw<NW>`: a demoted node re-checks compile/HBM/pace on ITS chips before touching an arm — the per-chip batch halves on the 8, the safe direction); the completion names its arm set and the optional arms it does not await (a demotion from the 32 drops A7/A8, from the 16 drops A6; their live state stays in `$GCS/live` for a re-promotion). Harness S12 (32 → 16 with A3 and A7 in flight: A3 resumes at its live step, A7 is left banked, the 16 re-preflights, completes with 7) and S12b (16 → 8 with A3 banked and A4 in flight: A3 skipped, A4 resumes, five preflights, completes with 6). Eval partials bank every 300 s (`--bank-every 300` on every sharded eval).
- **NIGHT A AT THE SOURCE (2026-09-05 13:00–13:30Z; the 32 did not land in us-east1-d, a 16 did):** (1) the preflight's inherited 2000-step warmup broke the cosine schedule at 60 steps → every core arm aborted at launch, A6 falsely skipped; fixed (`--warmup 10` after the arm flags; a preflight-shaped run on the real trainer), supervisor restarted with the fixed archive. (2) MEMORY: a wide arm at w384 needs 54.8 GB of HLO temporaries at 192 rows/chip → the one `--remat` retry fires by design and the arm trains; A6 (w512) does not fit at 192 rows/chip even with remat (712 MB free at program load) → SKIPPED, labeled; the parity arm needs gradient accumulation (2 × 384) — a build owed for Night B's B3. (3) PACE: 1.32 it/s for the w384 arm on 4 chips (metrics timestamps; the trainer's logged steps_per_sec reads 2× too high, and the preflight's figure is meaningless — an instrument bug to fix), so ≈ 10.5 h of training + ≈ 3.5 h battery per wide arm; the field-cell arms queued behind them would cross the cap while w3 idled → RE-MAP: w0 A3 · w1 A4 · w2 A5 · w3 A6(skip) A0 A1 A2, all inside the 06:25Z cap with ≈ 2.5 h margin; a preemption would consume it (an extension is the PI's call). Spend ≈ $210–240 for the night on the 16.
