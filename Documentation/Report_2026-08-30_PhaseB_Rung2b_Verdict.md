...
## 7. Addendum (2026-08-31) — the rider scans land: **B-M2 ACHIEVED**

The §5-recommended scans ran as registered (ledger 2026-08-30 RIDER SCANS registration; PHASE4-identical protocol, NSH=8 static, integrity all-PASS incl. byte-identical per-shard protocol fields ×16 and exact vote-convention identity on records; reader `tools/analyze_scan2b.py`, selftest 12/12; artifact `runs/analysis/sportBr2b_scan_verdict.txt`):

- **C3X 20k vote@128 = 88.89 % → B-M2 (≥ .85)** under both named conventions (draw-funnel 88.88). Prediction [84, 89] hit; +8.33 pp over the rung-2 carrier; +6.40 pp over the lens-B attempts-only forecast — §4.2's labeled ~86–88 extrapolation confirmed at its top edge, and **§2.6's "route to B-M2 is ρ via training" conclusion is now a measurement**. Curve unsaturated at k=128 (+10.0 pp on the last doubling); B-M3 (.95) is the open band.
- **D4 20k vote@128 = 81.28 %** (labeled row; prediction [76, 84] hit) — the cold-record arm carries a near-carrier-grade funnel too (above its 76.8 strat suggestion; the strat→20k offset is arm-dependent, both signs: C3 −7.9 / C3X −0.6 / D4 +4.5 pp). Its scan-cold 33.29 % replicates the 33.53 % record on the natural-dist subsample.
- Complementary profiles (paired, identical 20k set): C3X owns breadth (hard-bin vote 76.5 vs 64.5), D4 owns hard-cold (14.7 vs 2.8 %). **Portfolio fact (labeled, per §4.5): union vote = 95.08 %** — the program's first ≥.95 reading of any kind; the portfolio × verified-attempts protocol remains a registrable d128 lever.

Rung 2b is now fully closed (chain + audit + verdict + lens suite + rider scans). Scan-cell spend ≈$182 (2.0–2.4× the $75–90 estimate; churn + from-zero pole, named in the ledger entry). The claim-status table in §6 updates: **breadth 20k-grade = 88.89 % @128 (B-M2), C3X two-phase recipe, d96/2.11 M params.**
