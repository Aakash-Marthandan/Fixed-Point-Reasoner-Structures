...
### 7.4 Where the explosion lives (z-channel trace; `runs/analysis/sportC0_z_explosion_trace_20260902.txt`)
The carried-z recurrence is live on every arm (learned gate |α_z| 0.06–0.16, canvas included). On an exploding P2@50k trajectory |z_c| grows geometrically (median 2.6× per step once above 1e3: 480 @30 → 6.9e7 @40 → 1.2e15 @50 → 1.3e19 @55 → inf) while y stays on the simplex (max 1.0) until the logits — a linear readout of z — overflow. Every NaN class the program has seen lives in the free, unnormalized z channel: H-48's attention messages are computed from z; H-50's RI excursions grow through z. y is bounded by construction and its contraction is what FPA and the retention instruments manage. The principled stabilizer is a norm on z at the recurrence boundary (z_in = α_z·RMSNorm(z_c)·g, per-field shared so S9 is preserved, cfg-gated with off = bit-exact); the η cap is a correlate, not the mechanism. This is the stabilizer ladder's first rung as registered at rung 2b, now with its mechanism measured.
