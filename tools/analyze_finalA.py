...
# labeled; registration AMENDED pre-data 2026-09-05 on the PI's adversarial pass). AMENDMENT 2 (the PI: "why not 3M"):
# the core DEC arms at w384 = 2.78M (our champion's class); the wide arms' battery = LABELED subsamples (final/alt
# fulls on 50k, the D64 row on 100k, the scan 5k x k32: rho = verified@32 on 5k, not the X0-class column); the headline
# D16 val-selected full stays on all 422,786; DECODER-CLASS reads the D64 row's records. AMENDMENT 3 (the v6e-32, eight
# workers): A7 = A3 seed 1 (the DEC's OWN noise floor at w384) and A8 = A6 seed 1 (the parity arm's pair) — optional
# arms, run on the 32 only; the wide-arm contrasts use FLOOR_DEC = max(|cold16(A3) - cold16(A7)|, .005) when A7 exists,
# else the X0 floor (labeled).
...
#   INTEGRITY (a breach WITHHOLDS the verdict): X0-class arms: fulls n == 422,786 (vsel/final/alt at D16, vsel at
#     D64), scans n == 20,000 k128; wide arms: the D16 vsel full n == 422,786, final/alt n in {422,786 (copied), 50,000},
#     D64 n == 100,000, scans n == 5,000 k32; every scan: unique idx, identical idx sets within a class, protocol (t64,
#     seed 20260822), vote identity; every vsel-labeled eval of an arm reports ONE checkpoint path (the sportC1 lesson).
...
#   R-A-4 DECODER-CLASS (every arm, from the D64 row's per-puzzle cold column — 422,786 puzzles for the X0-class arms,
#     the 100k subsample for the wide arms): g50 = the logistic 50 % crossing of cold exact vs givens (None if no crossing
#     in 17..35 or no rising fit); YIELD = cold solve rate on rating > 0 puzzles.
...
#   R-A-7 PAIRS (descriptive + the floor): SPREAD-DEC = |cold16(A3) - cold16(A7)|, SPREAD-WIDE = |cold16(A6) - cold16(A8)|;
#     FLOOR_DEC = max(SPREAD-DEC, .005) governs R-A-0's A4-vs-A3 tolerance, R-A-3 and R-A-6 when A7 exists.
#   R-A-5 CARRY-TO-B (mechanical): CHAMP = argmax cold16 over STABLE non-memorized DEC arms {A3, A4, A5, A6, A7, A8};
#     B0 := A5's objective set iff R-A-3 in {HELPS, FLAT} and SPURIOUS(A5) <= .01 (missing -> A3's) else A3's;
#     W := 512 iff R-A-6 reads WIDTH-HELPS else 384; AUG := carried iff R-A-0 in {FAILS, PARTIAL} else dropped.
...
N_SCAN_W, K_W, N_FIN_W, N_D64_W = 5000, 32, 50000, 100000          # the wide arms' labeled battery
def is_wide(a): return a in DEC_ARMS
def n_scan(a): return N_SCAN_W if is_wide(a) else N_SCAN
def k_scan(a): return K_W if is_wide(a) else 128
def scan_proto(a): return dict(t_total=64, k_init=k_scan(a), subsample_seed=20260822)
HEADLINE_T = 16
PRED = {"A0": dict(cold16=(.850, .870)), "A1": dict(cold16=(.845, .875), spur=(0.0, .01), t1r=(.97, 1.0)), "A2": dict(cold16=(.850, .870)),
        ...
        "A7": dict(spread=(0.0, .015)), "A8": dict(spread=(0.0, .015))}
DESC = {"A0": "X0 seed 2 (noise floor pair)", "A1": "X0 + FPA anchor rows", "A2": "X0 + RI sigma 1",
        "A3": "DEC-w384, no digit aug", "A4": "DEC-w384 + digit aug (control)", "A5": "DEC-w384 + FPA + RI", "A6": "DEC-w512 (X0's class), remat",
        "A7": "A3 seed 1 (the DEC floor)", "A8": "A6 seed 1 (the parity pair)"}
LINES = []
...
def v128(a): s = scan(a); return None if not s else s.get("vote_at_k", {}).get(str(k_scan(a)))     # the class's k (128 / 32, labeled)
def t1r(a): s = scan(a); return None if not s else s.get("t1r_at_k", {}).get(str(k_scan(a)))
def d64_recs(a):
    q = evdir(a) / "full_vsel_t64" / "records_all.npz"; return dict(np.load(q, allow_pickle=True)) if q.exists() else None
def stopped(a):
...
def decoder_class(a, giv, rat):
    z = d64_recs(a)
    if z is None or giv is None or "cold_exact" not in z or "idx" not in z: return "NO-DATA", None, None
...
def integrity():
    errs = []
    for a in ARMS:
        exp = {("vsel", 16): {N_FULL}, ("final", 16): ({N_FULL, N_FIN_W} if is_wide(a) else {N_FULL}), ("vsel", 64): ({N_D64_W} if is_wide(a) else {N_FULL})}
        for (which, t), ns in exp.items():
            s_ = full(a, which, t)
            if s_ and s_["n"] not in ns: errs.append(f"{a} full_{which}_t{t} n={s_['n']} not in {sorted(ns)}")
        s_ = jload(evdir(a) / "full_vsel_t16_alt" / "summary_all.json")
        if s_ and s_["n"] not in exp[("final", 16)]: errs.append(f"{a} full_vsel_t16_alt n={s_['n']}")
    ...
    for a in ARMS:
        s_ = scan(a)
        if not s_: continue
        if s_["n"] != n_scan(a): errs.append(f"{a} scan n={s_['n']}!={n_scan(a)}")
        for k, v in scan_proto(a).items():
            if s_.get(k) != v: errs.append(f"{a} scan {k}={s_.get(k)}!={v}")
        z = scan_recs(a)
        if z is not None:
            ...
            if abs(float(hit.mean()) - s_["exact_acc_vote"]) > 5e-6: errs.append(f"{a} vote identity breach")
    for cls in (X_ARMS, DEC_ARMS):
        if len({idx_sets[a] for a in cls if a in idx_sets}) > 1: errs.append(f"scan idx sets differ within the class {cls}")
    return errs
...
def analyze():
    ...
    say("  arm | description                     | cold16 | final16 | cold64 |   b1   |  t1r@k  | v@k    | spur  | census | end-CE | stable | memorized | class (g50 / yield)   [k = 128 on 20k for A0-A2; 32 on 5k for the wide arms; cold64 on 100k for the wide arms]")
    rows = {}
    for a in ARMS:
    ...
    # R-A-7 PAIRS: the DEC's own floor (A3 vs A7) governs the wide-arm contrasts when it exists
    sd = None if None in (c["A3"], c["A7"]) else abs(c["A3"] - c["A7"]); sw = None if None in (c["A6"], c["A8"]) else abs(c["A6"] - c["A8"])
    if sd is not None: FLOOR_DEC = max(sd, .005); fdsrc = "DEC seed pair A3/A7"
    else: FLOOR_DEC = FLOOR; fdsrc = "the X0 floor (A7 absent, labeled)"
    V["FLOOR_DEC"] = FLOOR_DEC
    V["R-A-7"] = f"SPREAD-DEC:{'-' if sd is None else f'{sd:.4f}'}|SPREAD-WIDE:{'-' if sw is None else f'{sw:.4f}'}|FLOOR_DEC:{FLOOR_DEC:.4f}"
    say(f"R-A-7 PAIRS: {V['R-A-7']} ({fdsrc}); the wide-arm contrasts read beyond 2 x FLOOR_DEC = {2*FLOOR_DEC:.4f}")
    # R-A-0 ORBIT
    if c["A3"] is None: V["R-A-0"] = "NO-DATA"
    else:
        d = c["A3"] - X1_COLD16; tol = max(.02, 2 * FLOOR_DEC)
    ...
    if None in (c["A3"], c["A5"]): V["R-A-3"] = "NO-DATA"
        ...
        d = c["A5"] - c["A3"]
        V["R-A-3"] = "OBJ-HELPS" if d > 2 * FLOOR_DEC else ("OBJ-HURTS" if d < -2 * FLOOR_DEC else "OBJ-FLAT")
    ...
    if None in (c["A3"], c["A6"]): V["R-A-6"] = "NO-DATA" + ("" if c["A6"] is not None else " (A6 absent: 8-shape or preflight-skipped, labeled)")
        ...
        d = c["A6"] - c["A3"]
        V["R-A-6"] = "WIDTH-HELPS" if d > 2 * FLOOR_DEC else ("WIDTH-HURTS" if d < -2 * FLOOR_DEC else "WIDTH-FLAT")
    ...
    width = 512 if V["R-A-6"].startswith("WIDTH-HELPS") else 384
    ...
    sc = {"A0": hm(c["A0"], PRED["A0"]["cold16"]), "A1": f"cold {hm(c['A1'], PRED['A1']['cold16'])} spur {hm(s1, PRED['A1']['spur'])} t1r {hm(t1, PRED['A1']['t1r'])}",
          ...
          "A7": f"spread {hm(sd, PRED['A7']['spread'])}", "A8": f"spread {hm(sw, PRED['A8']['spread'])}"}
    for a in ARMS: say(f"  {a}: {sc[a]}")
    V["PRED"] = sc
    say("\nLETTERS: " + " · ".join(f"{k} {V[k]}" for k in ("INTEGRITY", "STABILITY", "MEMORIZATION", "R-A-0", "R-A-1", "R-A-2", "R-A-3", "R-A-6", "R-A-7", "R-A-5")))
    _write(); return V
...
# ---------- selftest ----------
def _npz(root, n=N_D64_W, seed=0):
    rng = np.random.default_rng(seed)
    want = rng.integers(17, 36, size=n)
    q = (rng.random((n, 81)) < (want / 81.0)[:, None]).astype(np.int8).reshape(n, 9, 9)
    giv = (q != 0).reshape(n, -1).sum(1)                       # the exact givens the analyzer will read
...
def _mk(root, a, giv, *, vc=None, fc=None, c64=None, b1v=None, v128v=None, t1rv=None, spur=0.005, ce=0.5, census_=0.0, stopped_=None,
        n_scan_override=None, ckpt="ckpt_040000.pkl", scan_ckpt=None, soft=False, seed=1):
    rng = np.random.default_rng(seed); ev = root / f"sxeval_p{TAG}{a}"; ck = f"runs/pretrain{TAG}_{a}/{ckpt}"
    wide = is_wide(a); nd64 = N_D64_W if wide else N_FULL; nfin = N_FIN_W if wide else N_FULL
    for which, v, t, nn in (("vsel", vc, 16, N_FULL), ("final", fc, 16, nfin), ("vsel", c64, 64, nd64)):
        if v is not None:
            d = ev / f"full_{which}_t{t}"; d.mkdir(parents=True, exist_ok=True)
            (d / "summary_all.json").write_text(json.dumps(dict(exact_acc=v, n=nn, ckpt=ck)))
            if t == 64:   # the D64 row's per-puzzle records feed DECODER-CLASS
                m = min(nn, len(giv)); idx64 = np.arange(m)
                cold64 = (((giv[idx64] >= 26) & (rng.random(m) < .6)) ^ (rng.random(m) < .03)) if soft else (rng.random(m) < v)
                np.savez(d / "records_all.npz", idx=idx64, cold_exact=cold64, rating=np.zeros(m))
    if vc is not None:
        d = ev / "full_vsel_t16_alt"; d.mkdir(parents=True, exist_ok=True); (d / "summary_all.json").write_text(json.dumps(dict(exact_acc=vc, n=nfin, ckpt=ck)))
    if b1v is not None:
        d = root / f"sxscan_p{TAG}{a}"; d.mkdir(parents=True, exist_ok=True); n = n_scan(a) if n_scan_override is None else n_scan_override; idx = np.arange(n)
        ...
        kk = str(k_scan(a))
        (d / "summary_all.json").write_text(json.dumps(dict(n=n, t_total=64, k_init=k_scan(a), subsample_seed=20260822, b1_exact=b1v, exact_acc=float(cold.mean()),
                                                            exact_acc_vote=vote, vote_at_k={kk: vote}, t1r_at_k={kk: t1rv if t1rv is not None else vote * .99}, ckpt=(scan_ckpt or ck))))
...
def selftest():
    ...
    def wA(r, g):  # the hoped-for world: exact S9 replaces aug at parity; the selector lifts free; RI helps; objectives help; decimating class
        ...
        _mk(r, "A7", g, vc=.854, fc=.854, c64=.926, b1v=.91, v128v=.996, t1rv=.92, spur=.05)
        _mk(r, "A8", g, vc=.885, fc=.885, c64=.938, b1v=.93, v128v=.998, t1rv=.93, spur=.05)
    v = run(wA)
    checks += [("A integrity", v["INTEGRITY"] == "PASS"), ("A floor = the .005 clamp (|A0 - X0| = .0023 below it)", v["FLOOR"] == .005),
               ...
               ("A pairs: the DEC floor from A3/A7 (.005 clamp), spreads read", v["R-A-7"] == "SPREAD-DEC:0.0040|SPREAD-WIDE:0.0050|FLOOR_DEC:0.0050"),
    ...
    checks += [("B orbit fails", v["R-A-0"] == "EXACT-S9-FAILS"), ("B selector flat", v["R-A-1"] == "LIFT-FLAT"), ("B RI hurts", v["R-A-2"] == "RI-HURTS"),
               ...
               ("B pairs absent -> the X0 floor governs (labeled)", v["R-A-7"].startswith("SPREAD-DEC:-|SPREAD-WIDE:-|") and v["FLOOR_DEC"] == v["FLOOR"]),
               ("B carry: champ A4 (A3 memorized excluded), B0 := A3, w384, aug carried", v["R-A-5"] == "CHAMP:A4|B0:=A3|W:=384|AUG:carried")]
    ...
    def wC2(r, g):  # a wide arm scanned at the X0-class size = a protocol breach (the per-class gate)
        wA(r, g); _mk(r, "A3", g, vc=.85, fc=.85, c64=.925, b1v=.91, v128v=.996, t1rv=.92, spur=.05, n_scan_override=N_SCAN)
    v = run(wC2)
    checks += [("C2 wide-arm scan at 20k = protocol breach, withheld", v["INTEGRITY"] == "FAIL")]
    def wG(r, g):  # a wide DEC seed pair with a LARGE spread (2 pp) widens the wide-arm tolerance: A5 +2.5 pp reads FLAT, not HELPS
        wA(r, g); _mk(r, "A7", g, vc=.83, fc=.83, c64=.92, b1v=.90, v128v=.995, t1rv=.92, spur=.05)
        _mk(r, "A5", g, vc=.875, fc=.875, c64=.935, b1v=.92, v128v=.998, t1rv=.99, spur=.003)
    v = run(wG)
    checks += [("G a 2-pp DEC spread makes a 2.5-pp objective gain FLAT (the DEC floor governs)", v["R-A-3"] == "OBJ-FLAT+SELECTOR-CLEAN" and abs(v["FLOOR_DEC"] - .02) < 1e-9)]
    ...
    checks += [("D floor default", v["FLOOR"] == .01), ("D orbit partial (aug still adds 5 pp)", v["R-A-0"] == "EXACT-S9-PARTIAL"),
               ...
               ("D carry: champ A4, B0 := A3, w384, aug carried", v["R-A-5"] == "CHAMP:A4|B0:=A3|W:=384|AUG:carried")]
    v = run(lambda r, g: None)
    checks += [("E no data at all", v["R-A-0"] == "NO-DATA" and v["R-A-5"] == "CHAMP:none|B0:=A3|W:=384|AUG:dropped")]
    ...
    v = run(wF)
    checks += [("F width flat -> w384 carried, parity tags still on the matched arm", v["R-A-6"] == "WIDTH-FLAT+PARITY-AT-D16+PARITY" and "|W:=384|" in v["R-A-5"])]
