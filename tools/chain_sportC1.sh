#!/bin/bash
# Ledger: CHAMPION sportC1 chain (2026-09-02; Plan_2026-09-02_Champion_sportC1
# §4 / §11.5 / §12). Derivative of the proven chain_sportC0.sh (static worker
...
GCS=${GCS:-gs://qhrrn2-rescue/sportC1}
...
x_common () {       # the field-recipe baseline: TRM/EqR cell + SOT loop in the field's regime (plan §11.2)
  echo "--sudoku-extreme $NPZ --sudoku-layout native9 --equilibrium --cell trm --sot \
        ...
        --trm-lambda 0.05 --trm-beta 0.01 --sudoku-aug 1000 --sudoku-digit-aug --loss stablemax \
...
arm_flags () {
  case $1 in
    A0)  echo "$(native_common) $(our_regime) --seed 0";;
    A1)  echo "$(native_common) $(our_regime) --seed 1";;
    B0)  echo "$(native_common) $(our_regime) --seed 0 --z-norm rms";;
    B1)  echo "$(native_common) $(our_regime) --seed 1 --z-norm rms";;
    R0)  echo "$(native_common) $(field_regime) --seed 0 --z-norm rms --remat";;
    X0)  echo "$(x_common) --act --seed 0";;
    X0n) echo "$(x_common) --seed 0";;
    *)   return 1;;
  esac
}
two_stage () { case $1 in A0|A1|B0|B1) return 0;; *) return 1;; esac; }
is_field ()  { case $1 in X0|X0n) return 0;; *) return 1;; esac; }
arm_steps () { case $1 in R0) echo "$STEPS_R";; X0|X0n) echo "$STEPS_X";; *) echo "$STEPS_AB";; esac; }
arm_prec ()  { case $1 in X0|X0n) echo default;; *) echo highest;; esac; }   # the field trains/evals in bf16
head_ema ()  { case $1 in R0|X0|X0n) echo "--ema";; *) echo "";; esac; }     # headline weights (plan §12.4)
alt_ema ()   { case $1 in R0|X0|X0n) echo "";; *) echo "--ema";; esac; }
select_key (){ case $1 in R0) echo val_t64_ema;; X0|X0n) echo val_t16_ema;; *) echo val_t64;; esac; }
head_t ()    { if is_field "$1"; then echo 16; else echo 64; fi; }             # X0 headline = EqR's D=16
screen_steps () {  # fixed-step screens (vb rides separately); two-stage: stage-A end + B+5k + B+15k
  case $1 in R0|X0|X0n) echo "015000 035000";; *) echo "";; esac; }
...
riders () {  # canvas C3X/D4 EqR-statistic evals (program review §1) — owed since the pilot's early close
  ARM_PREC=highest
...
if [ "$NW" -ge 4 ]; then
  case $W in
    0) run_arm A0 || rc=1; run_arm B0 || rc=1;;
    1) run_arm A1 || rc=1; run_arm B1 || rc=1;;
    2) run_arm R0 || rc=1;;
    3) run_arm X0 || rc=1; run_arm X0n || rc=1; riders || rc=1;;
  esac
else
  for arm in A0 A1 B0 B1 X0 X0n R0; do run_arm "$arm" || rc=1; done
  riders || rc=1
fi

# ---------- completion (any worker; idempotent) ----------
need="A0 A1 B0 B1 R0 X0 X0n"
for pass in $(seq 1 "${C1_WAIT_PASSES:-200}"); do
  all=1
  for armx in $need; do gsutil -q stat "$GCS/${armx}_ARM_OK" 2>/dev/null || all=0; done
  if [ "$all" -eq 1 ]; then
    if ! gsutil -q stat "$GCS/sportC1_final.tgz" 2>/dev/null; then
      ...
      tar czf /tmp/sportC1_final.tgz runs/pretrain${R_TAG}_* runs/sxscreen_p${R_TAG}* runs/sxeval_p${R_TAG}* runs/sxscan_p${R_TAG}* runs/sxcensus_p${R_TAG}* runs/sxrider_* 2>/dev/null
      gsutil -q cp /tmp/sportC1_final.tgz "$GCS/sportC1_final.tgz"
    fi
