...
while :; do
  ...
  if [ -n "$DL" ] && [ "$DL" -gt "$now" ]; then
    ...
    usecs=$(perl -e 'alarm 90; exec @ARGV' -- gcloud compute tpus tpu-vm ssh "$POD" --zone "$Z" --project ${PROJECT:-quantum-llm} --worker all --ssh-flag "-o StrictHostKeyChecking=no" --ssh-flag "-o UserKnownHostsFile=/dev/null" --ssh-flag "-o ConnectTimeout=20" --command "grep -oE 'USEC=[0-9]+' /run/systemd/shutdown/scheduled 2>/dev/null | cut -d= -f2" 2>/dev/null | grep -oE '[0-9]{10,}')
    sched=$(echo "$usecs" | awk '{s=int($1/1000000); if(m==""||s<m)m=s} END{print (m==""?0:m)}')
    if [ "$sched" -lt $((DL + 1800)) ]; then
      dms=$(( (DL - now) / 60 + 60 ))
      out=$(perl -e 'alarm 120; exec @ARGV' -- gcloud compute tpus tpu-vm ssh "$POD" --zone "$Z" --project ${PROJECT:-quantum-llm} --worker all --ssh-flag "-o StrictHostKeyChecking=no" --ssh-flag "-o UserKnownHostsFile=/dev/null" --ssh-flag "-o ConnectTimeout=20" --command "sudo shutdown -c 2>/dev/null; sudo shutdown -h +$dms >/dev/null 2>&1 && echo PUSHED-\$(hostname|sed 's/.*-w-//')" 2>/dev/null | grep -oE 'PUSHED-[0-9]' | tr '\n' ' ')
