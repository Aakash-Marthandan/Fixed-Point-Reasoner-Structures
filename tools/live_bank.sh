...
set -uo pipefail
cd "$(dirname "$0")/.." || exit 1
GCS=${GCS:-gs://qhrrn2-rescue/sportC2}; LIVE="${LIVE_PREFIX:-$GCS/live}"; R_TAG=${R_TAG:-sportC2}   # LIVE_PREFIX (2026-09-13): bank to a fresh prefix while the markers stay under GCS (unset = byte-identical)
