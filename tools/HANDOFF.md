...
## ✅ ANALYZED — CHAMPION TRACK PILOT sportC0 (ops closed 2026-09-01 by the PI's early close; node DELETED, fleet ZERO 8 zones; analysis pass 2026-09-02 Fable): data pulled serially (`gcloud storage cp` + crc32c per object — the first `gsutil cp` stalled at full size with a WRONG checksum; never trust size alone), extracted into `runs/`; `analyze_sportC0.py` untouched (0-diff vs 6679ac2, 16/16) → `runs/analysis/sportC0_verdict.txt`: STABILITY UNCLEAN P2s1/P3/P5/P6 · R-C0-1 ARITY-HURTS (letter; read = memorization collapse of the native regime, geometry exonerated at val-selected ckpts) · R-C0-2 RI-FRAGILE-AT-WIDTH · R-C0-3 NI-NEUTRAL · R-C0-4 NO-DATA (P6 scan never ran) · R-C0-5 GO-FALLBACK:P3s1 (= the champion recipe, seed 1; cold 37.35 program record, b1 38.47, t1r@128 52.49). Physics pass + report `Report_2026-09-02_ChampionPilot_Verdict.md`; champion design `Plan_2026-09-02_Champion_sportC1.md` (registration draft; launch on PI go after analyzer/harness lock). Ops lessons folded into the plan §4. The block below is the historical ops record.

## 🗄 SUPERSEDED — OPS HANDOFF (Opus) as of 2026-09-01 ~08:25Z — CHAMPION TRACK PILOT (sportC0): seven native9 d96 arms, one night; supervisor riding, strike 1/2 absorbed, re-create in flight

**Your job:** ride the pilot to `CHAIN-SPORTC0-COMPLETE` (+ `sportC0_final.tgz` in `gs://qhrrn2-rescue/sportC0`) → verify SELF-TEARDOWN (positive `describe` = NOT_FOUND) → FLEET-ZERO positive describes (us-east1-d, us-east5-b, us-central1-a, us-central2-b, us-west1-c, asia-south1-b/c, asia-east1-c; nodes + QRs) → pull the final tgz → extract into `runs/` → COUNTS-ONLY checks → factual ops report → STOP. **HARD BOUNDARY: no accuracy values, no vote numbers, no adjudication, no ledger verdicts, `tools/analyze_sportC0.py` untouched, chain FROZEN (B5; live verbs = kill-orphan / extend-deadline-with-PI-go / pause). The verdict + the sportC1 champion-d128 registration are the analysis pass (Fable, PI-sequenced).** Honest measured ETAs every heartbeat; escalate the moment a projection slips; never mask a diagnostic.

**STATE UPDATE (10:15Z): MUMBAI SWITCH — PI: "use a mumbai v6e-8 pod" after TWO US-day preemptions (v6e-16 08:18Z pre-chain + v6e-8 10:10Z mid-P1-scan). `campaign.env` now ZONES='asia-south1-c asia-south1-b' ACCEL/ACCEL_LIST=v6e-8 (PI override of US-first for this churny window; asia-south1-c = the proven-stable source). Supervisor RESTARTED pid 32114 (Mumbai hunt, deadline 03:00Z), caffeinate re-pinned, code archive re-banked, guard re-armed for the Mumbai node. P1 fully banked except the 20k scan (5 eval _OK markers in GCS: 3 screens + retfm + full_t64) — resumes on the new node (markers skip the done pieces). Fleet was ZERO at switch (clean). ETA COMPLETE ~17:00–18:30Z; spend ~$60 total (~$4 US churn + ~$56 Mumbai @ ~$8/h). Everything below is the launch-time truth; the Mumbai switch supersedes the zone/accel lines.**

**STATE (measured 08:28Z, Opus driving):** registration = ledger §5 2026-09-01 "CHAMPION TRACK PILOT — LAUNCH REGISTRATION" (pre-data; predictions + law-interaction matrix live there; design = `Plan_2026-09-01_Champion_Track.md`). Supervisor pid in `runs/pod_supervisor.pid` (launched 08:04:47Z, poll 300s, caffeinate pinned); code archive `code_6679ac2dirty0804.tgz` banked (bring-ups pull from GCS). Timeline: v6e-16 CREATED us-east1-d 08:10Z → **PREEMPTED 08:18Z pre-chain (strike 1/2)** → east5-b/west1-c 16s no-capacity → **fell to v6e-8, CREATING in us-east1-d from 08:25:16Z (1×8: ALL 7 arms SEQUENTIAL on one worker, DP-8).** **PI DECISION 08:28Z: proceed on the 8 + extend the deadline (the 8's ~12–16h wall would clip the launch+12h cap).** Deadline knob `runs/tpu_deadline.txt` now = **1788318000 (2026-09-02 03:00:00Z = 08:30 IST)**; watchdog + supervisor both follow it; worst-case unattended spend cap ≈ $126 (self-teardown fires on COMPLETE first). `tools/plant_guard.sh 24` RESTARTED against the NEW deadline (the prior loop had captured the old epoch — restart on every deadline change), background, still waiting for READY. **Any FURTHER deadline change: update the knob, then kill + restart plant_guard so it plants the new epoch.** Spend so far ≈ $2.

**EXPECTED SEQUENCE:** node READY → supervisor bootstraps (GCS code+venv) → **CANARY with the B1 PROJECTION GATE: all pilot cost figures are INFERRED from a ~12× native step-compute estimate — the canary's measured it/s re-prices the night; if projected node-spend > 1.2× the $85–125 (16) / $80–110 (8) band, HOLD (supervisor pauses on gate fail) and report to the PI** → chain launches per worker on the STATIC map (4×4: W0 P1,P6 · W1 P2,P2s1 · W2 P3,P5 · W3 P3s1+riders; 1×8: sequential). Per arm: `PRETRAIN-START/OK` (two-stage arms P3/P3s1/P6 emit a stage-A `STAGEA_OK` marker first) → screens/retfm/full/scan evals with `EVAL-OK` echoes → `ARM-OK` marker in GCS. Rough wall at the estimate: ~2–2.5h/arm-pair on the 16 → COMPLETE ≈ T+6–9h from chain start; **recompute from the canary's measured pace and per-arm `PRETRAIN-OK` timestamps — never quote the estimate as measured.** Riders (canvas C3X/D4 sel-5k evals) ride W3.

**DECISION TABLE:** `PRETRAIN-NAN <arm> -> amputate` = a REGISTERED outcome (one-shot rule; amputation banks the last finite grid + STOPPED label; the chain continues — do NOT retry the arm) · `AMPUTATE-FAILED` = STOP AND REPORT · `EVAL-N-BAD` / `EVAL-SHARD-FAILED` = read that shard log AT THE NODE immediately; N-BAD = partition integrity, stop and report, delete nothing · preemption → supervisor re-hunts (strike 2 on the 16 ⇒ DEMOTED to v6e-8 for the supervisor's life); on the NEW node: partials auto-resume (arm-level `PRETRAIN_OK`/`STAGEA_OK`/eval markers + 300s eval partials bound loss to ≤10 min) — **re-plant the guard + re-push the DMS (plant_guard.sh)** · `SPORTC0-INCOMPLETE` = wait-passes exhausted with arms missing — read WHY (which `*_ARM_OK` is absent, then that worker's log) before any relaunch · deadline binding → the guard clips = a labeled pause, banked partials make it cheap; extension = PI-GATED ONLY (then re-run plant_guard for the new epoch) · Mumbai/asia = US-dry fallback ONLY (standing PI policy).

**SSH RULES (each a paid-for lesson, 2026-08-30/31):** (1) always `--ssh-flag "-o StrictHostKeyChecking=no" --ssh-flag "-o UserKnownHostsFile=/dev/null"` (reused-IP 255 class). (2) NEVER put a `pkill` in the same ssh command as a payload containing its own pattern text (the v_stop self-match class) — plant_guard.sh embodies the two-call pattern; use it rather than hand-rolling. (3) Bound every remote call (`perl -e 'alarm N; exec @ARGV' --`). (4) Diagnostics with stderr VISIBLE, and **never judge success through a `| tail` pipe — read the rc directly** (re-bitten 2026-09-01: a piped ssh masked a connect-timeout as rc=0). (5) Before any manual eval: fetch + `ls`-verify its dependencies (ckpt, npz). (6) Verify by ARTIFACT (GCS objects, node logs, `describe` states), never wrapper pgrep.

**RE-ARM AFTER ANY APP RESTART (session-scoped layers die; supervisor + caffeinate + launchd watchdog survive):** (a) edge monitor — tail -F `runs/pod_qhrrn2-pod2.log` filtered to `CREATED|READY|CANARY|PREEMPT|DEMOT|PRETRAIN-NAN|EVAL-N-BAD|MERGE-N-BAD|INCOMPLETE|COMPLETE|FAILED|TEARDOWN|needs eyes|deadline`; (b) hourly heartbeat to the PI — per-process completion fraction + concrete ETA from measured pace (arm markers in GCS + `PRETRAIN-OK` timestamps + monitor `step` rows), IST with UTC in parentheses, spend vs the $85–125 band (watchdog spans × $13.64 v6e-16 / $6.82 v6e-8 — NOTE `spend_report.py` over-rates US-16 spans at the Mumbai 2.35× multiplier, hand-derive at US rates); (c) verify `launchctl list | grep qhrrn2` (watchdog) and the guard (`pgrep` on the node via a bounded ssh, or re-run plant_guard — idempotent).

**ON COMPLETE (§-close):** sentinel in the pod log + `sportC0_final.tgz` in GCS verified at source → node positive-describe ABSENT → fleet-zero sweep (8 zones, nodes + QRs) → `gcloud storage cp` the final tgz → extract into `runs/` → COUNTS ONLY: 7 pretrain dirs with `ckpt_latest.pkl` (+ any `STOPPED.txt` noted verbatim), per-arm screen/retfm/full/scan dirs with `summary_all.json` (full n==422786, scan n==20000, screens/retfm n==512 — counts, NO value reads), 2 rider dirs (n==5000), `*_ARM_OK` ×7 in GCS → stop session layers (leave the launchd watchdog) → ONE factual ops report (completion time, node timeline, incidents, spend hand-derived, "analysis pass ready — Fable adjudicates R-C0-1..5 on PI go"). NO commit (the analysis pass carries the ledger annotation + commit).

## ✅ COMPLETE — RUNG 2B RIDER SCANS (2026-08-31 12:01:08Z `p4x_COMPLETE.txt` + sentinel + runbook SELF-TEARDOWN [node ABSENT 12:05Z]; fleet-zero verified 8 zones nodes+QRs; session layers verified down; both tgzs pulled + extracted → `runs/sxscan_psportBr2bC3X/` + `...D4/`, `summary_all.json` n==20000 each). **VERDICT (analysis pass, 2026-08-31): B-M2 ACHIEVED — C3X 20k vote@128 = 88.89% (≥ .85 under both named conventions; prediction [84,89] HIT); D4 = 81.28% labeled ([76,84] HIT); integrity all-PASS incl. byte-identical per-shard protocol ×16 (the manual-eval acceleration proven benign at artifact level); reader `tools/analyze_scan2b.py` selftest 12/12 → `runs/analysis/sportBr2b_scan_verdict.txt`; the ledger §5 2026-08-31 SCAN VERDICT entry = THE record; Report_2026-08-30 §7 addendum.** Scan-cell spend ≈$182 (US rates from watchdog spans; 2.0–2.4× the $75–90 estimate — 16-churn ×3 recreates, overnight gap, s7 from-zero pole, all named; spend_report zone-aware 16-rate fix owed). **RUNG 2B FULLY CLOSED** (chain + audit 286/0/0 + verdict + lens suite + rider scans). NEXT: the FREETHINK (staged inputs) → start-to-now program review → d128 registration (breadth C3X-class ×2, deep D3-lane @1e-3, B-M3 the open band).

## 🗄 SUPERSEDED (2026-08-31 05:55Z live ops block — scans completed 12:01Z; the COMPLETE block above + ledger entry are the record) — RIDER SCANS resume on a v6e-8 (us-east1-d), 7 evals mid-flight, COMPLETE ≈ 10:15–10:45Z

**Your job:** ride the two 20k scans (C3X + D4) to `SCAN-SPORTBR2B-COMPLETE` (+ `p4x_COMPLETE.txt` in GCS) → verify SELF-TEARDOWN (positive `describe` = NOT_FOUND) → FLEET-ZERO positive describes (us-east1-d, us-east5-b, us-central1-a, us-central2-b, us-west1-c, asia-south1-b/c, asia-east1-c; nodes + QRs) → pull `gs://qhrrn2-rescue/sportBr2b/p4x_C3X.tgz` + `p4x_D4.tgz` → extract into `runs/` → report to the PI → STOP. **HARD BOUNDARY: no accuracy values, no vote numbers, no adjudication, no ledger verdicts — the B-M2 read + ledger annotation + commit are the analysis pass (Fable, PI-sequenced). Your close deliverable = completion + teardown + pull + a factual ops report.** PI is BUDGET-SENSITIVE and error-weary today: honest measured ETAs every heartbeat, escalate the moment a projection slips, never mask a diagnostic.

**STATE (measured 05:49Z):** registration = ledger "RUNG 2B RIDER SCANS" (2026-08-30, pre-data; PHASE4-identical flags; NSH=8/scan; predictions locked). Runbook `tools/scan_runbook_sportBr2b.sh` (harness 13/13) runs 1×8: C3X block then D4 block. GCS summaries: C3X 6/8 (s3 @97% banks momentarily; **s5 = C3X pole, 41%, ~8.3 rec/min → done ≈08:50Z**), D4 2/8 banked (s1, s3). **SIX D4 evals run MANUALLY outside the runbook** (PI-approved acceleration on idle chips; self-banking wrappers push summary+records to `p4xd4/` on completion): s0/s2 @72%, s4/s5/s6 @77% (done ≈07:00Z), **s7 from-zero @10% = the global pole → done ≈09:30–10:30Z**. Deadline knob `runs/tpu_deadline.txt` = 1788180571 (**12:49:31Z**) — watchdog + supervisor follow it; **on-node delete guard armed** (verified; fires at that epoch; the Mac-independent billing backstop); DMS (guest shutdown, NOT a billing backstop) parked 12:43Z. Supervisor pid `runs/pod_supervisor.pid` + caffeinate live; heartbeat cron :53; edge monitor session-scoped (re-arm after any app restart: tail -F the pod log for COMPLETE/FAILED/PREEMPT/CREATED/MERGE-N-BAD/INCOMPLETE). Spend: this resume ≈$18 so far at $6.82/h (from 03:25Z); scans-cell total to date ≈$150.

**EXPECTED SEQUENCE:** D4 manuals bank s0/s2/s4/s5/s6 (~07:00Z) → C3X s5 banks (~08:50Z) → runbook merges C3X (n-gated ==20000) → `SCAN-C3X-OK` → **runbook's own D4 wave spawns**: banked shards `SHARD-SKIP`; s7 (if still unbanked) gets a runbook DUPLICATE of the manual — **BENIGN-BY-DETERMINISM** (identical protocol/seed ⇒ identical records; first bank wins; do NOT kill either) → s7 banks (~09:30–10:30Z) → `SCAN-D4-OK` → `p4x_COMPLETE.txt` + sentinel + SELF-TEARDOWN. Merges are idempotent + n-gated; `MERGE-WAIT n/8` = normal polling; `MERGE-N-BAD` = STOP AND REPORT (partition integrity — do not delete anything).

**DECISION TABLE:** `SHARD-OK/-SKIP` normal · `SHARD-WAIT (chip busy)` normal ≤~6 cycles · `SHARD-FAILED` → read that shard log AT THE NODE immediately (no waiting a cycle) · preemption → supervisor rehunts **v6e-8 ONLY** (env pinned; archive `code_f701924dirty0319.tgz` carries every fix: npz-fetch, cache-restore, chain-verbatim pin) → on the NEW node: partials auto-resume (≤10 min loss); **re-plant the guard + push DMS** (below) · `INCOMPLETE` → report, do not relaunch without reading why · deadline binding → the guard clips = labeled pause; extension = **PI-GATED ONLY**.

**SSH RULES (each is a paid-for lesson, 2026-08-30/31):** (1) always `--ssh-flag "-o StrictHostKeyChecking=no" --ssh-flag "-o UserKnownHostsFile=/dev/null"` (reused-IP stale-host-key 255 class). (2) **NEVER put a `pkill` in the same ssh command as a payload containing its pattern text — the pkill kills its own session** (the v_stop self-match class, re-bitten twice tonight): guard replacement = TWO calls — call 1 `pkill -f "tpu-vm [d]elete qhrrn2-pod2"` + `sudo shutdown -c; sudo shutdown -h +NNN`; call 2 plant: `SECS=$((DEADLINE_EPOCH - $(date +%s))); setsid nohup bash -c "sleep $SECS; gcloud compute tpus tpu-vm delete qhrrn2-pod2 --zone=<zone> --project=quantum-llm --quiet" </dev/null >/tmp/deadline_guard.log 2>&1 & pgrep -f "tpu-vm [d]elete"` (verify a pid). (3) Bound every remote call (`perl -e 'alarm N; exec @ARGV' --`). (4) Diagnostics with stderr VISIBLE (no `2>/dev/null` when reading failures). (5) Before ANY manual eval: fetch + `ls`-verify its dependencies (ckpt, npz) — two incidents tonight were lazy-fetch assumptions. (6) Verify by artifact (GCS objects, node logs), never wrapper pgrep.

**ON COMPLETE (your §-close):** verify sentinel in the pod log + `p4x_COMPLETE.txt`; positive-describe the node ABSENT; fleet-zero sweep (8 zones, nodes + QRs); `gcloud storage cp` both tgzs → extract into `runs/` (`runs/sxscan_psportBr2bC3X/` + `...D4/` with `summary_all.json` n==20000 each — COUNT check only, no value reads); stop session layers; report: completion time, spend (node-hours × $6.82 + the ride ledger), incident-free-or-not, "analysis pass ready — Fable adjudicates B-M2 on PI go". ONE factual ops report; no commit (the analysis pass carries the ledger annotation + commit).

## ✅ COMPLETE — PHASE B RUNG 2B (2026-08-30 12:52:05Z chain-complete + self-teardown + supervisor exit 0; FLEET ZERO positive-described 11 zones ×1 at close; STRICT FINAL AUDIT --tag sportBr2b --phase final = 286/0/0 = `runs/audit_sportBr2b_final_20260830.txt`; frozen-tag regression sportBr2 329/0/0 re-verified). The 2026-08-30 ledger §5 OPS CLOSE entry is THE record (five-node ride, 16→8 cost switch, two preemptions + canary transport-flake + stale-host-key + orphan-claim incidents all registered with fixes, node-side delete-guard procedure, rate recalibration [Mumbai-16 ≈$16/h, US-16 $13.64], spend ≈$255-265 inside the $200-270 band, three PI-gated extensions, PI policy: Mumbai = US-dry fallback only). Final data: `runs/` (extracted, canonical, 852 MB tgz) + `gs://qhrrn2-rescue/sportBr2b`. D3+D4 dose cells CLEAN through 50k at both lrs; D1 STOPPED@10k labeled; PHASE4 gate FAIL banked (legit absence). ANALYSIS = ON PI GO: `analyze_sportBr2b.py` untouched (self-test 24/24 incl. C3X R2b-5), fresh ingestion → verdict → d128 registration per the carrier rules; the FREETHINK rides the same session per the PI's morning plan. All session layers/crons/monitors STOPPED; launchd watchdog stands (deadline 13:26:12Z Aug 30 — nothing up).

## ⏸ SUPERSEDED (2026-08-29 close block) — SESSION CLOSED ~19:30Z 2026-08-29 — CAMPAIGN RIDES OVERNIGHT UNATTENDED (BY DESIGN). NEXT SESSION PICKUP ORDER (PI-set): (1) THE FREETHINK FIRST — "long overdue": QG / information-theory / RG / holography synthesis over the session's findings (seam-mixers carry 72–85% of ALL learning [the PI's holography flag: boundary-acting components = where the code lives], η-surge→settle→IGNITION sequence [the PI wants the η↔ignition relationship understood], rule_H de-commitment ↔ draw-diversity [C4], the (ρ,r) funnel plane + ignition-as-ρ-expansion, toll-narrows-basins-with-scale [D8 inversion], the 61.4% cold hard core + portfolio law, unheralded-cliff NaNs + H-48, Fisher consolidation clock, graded-radius-vs-scale) — literature research included; write a COMPLETE freethink into Research_Brainstorm.md (the two partial 08-29 entries were rewind debris, cleaned; lettering continues from W). (2) OPS: chain COMPLETE expected ≈03:45–04:30Z with SELF-TEARDOWN; verify at source, then §6 close (pull final tgz → counts → `audit_sportB_integrity.py --tag sportBr2b --phase final` 0-FAIL → fleet-zero POSITIVE-describes 11 zones → ONE ledger line + ONE commit). If NOT complete at pickup: supervisor (Mac pid in runs/pod_supervisor.pid) rode it — read its log, verify at source, extend NOTHING without checking the B2 cap (deadline 09:26Z Aug 30 = the watchdog hard-deletes past it; the banked partials make any cap-stop a labeled pause, not a loss; PI go required to extend). (3) 2b ANALYSIS on PI go (fresh ingestion; `analyze_sportBr2b.py` untouched, selftest 24/24 incl. the C3X addendum rules R2b-5) → 2c decision (dosed-D1-redux / stabilizer cells per H-48's answer) → d128 registration (use the (ρ,r) readout + the lens-B saturation logic; deep lane iff D3 clean). OVERNIGHT SAFETY = supervisor (survives app exit) + caffeinate + launchd watchdog + B2 deadline cap + chain self-teardown + one-shot-NaN amputation — no session layers needed; max unattended spend ≈ $300 by construction. Session cron/monitors died with the session (expected).

## ▶ LIVE — PHASE B RUNG 2B (launched 2026-08-29 evening IST; ledger "PHASE B RUNG 2B LAUNCH REGISTRATION" = the science truth; Plan_2026-08-29_Rung2b.md §5/§5b = the ops design)
**VERDICT-ITEMS REGISTER (accumulating during the ride):** (1) D1 STOPPED @10k (NaN @10.8k; auto-amputated; labeled) — the schedule/lr law: free-T6 on the 50k cosine dies where the same recipe on the 20k cosine (C3, D2) trains clean; instability = free-attention-flux × effective-lr at width, NOT depth — feeds H-48's mechanism + the d128 design. (2) **C3-CONTINUATION RIDER (PI-noted 2026-08-29): low-lr continuation from C3's banked 20k ckpt, +30k T6 steps ≈ $30-40 — the surviving B-M2 route; paired with D1's death it brackets the lr mechanism (fresh-hot dies vs continue-cool survives?); decide at the verdict (may be moot if D3/D4 deliver).** (3) STABILIZER ESCALATION REGISTER (PI normalization question): if the β_nl dose fails → z-clamp/renorm (cfg-gated; NI-renorm precedent) → η_z/z-gate damping (existing dials) → RMSNorm on attention messages → full FPRM pre-norm kit as a NEW BASELINE with a twin-arm bridge cell; coupled residual scaling stays dead on the y-simplex (measured 2×) but is legitimate on the free z-channel (noted option). Formalize all three in the 2b verdict entry.**

**AUTONOMOUS MODE (PI, 2026-08-29 ~17:05 IST / 11:35Z): ops owned to CHAIN-SPORTBR2B-COMPLETE + §6 close (counts + audit --tag sportBr2b 0-FAIL + fleet-zero positive-describe + ONE ledger line + ONE commit), then STOP. Science gates unchanged (analyzer untouched; one-shot arms — a NaN is registered science, never an incident to fix). The B2 deadline cap (09:26Z Aug 30) remains PI-GATED — autonomous mode does NOT extend it. PI update owed at: all-four-arms-verified-running, then heartbeats.**
Campaign: tag `sportBr2b`, GCS `gs://qhrrn2-rescue/sportBr2b`, chain `tools/chain_sportBr2b.sh` (harness 38/38), env `tools/campaign_sportBr2b.env` → `tools/campaign.env`. ARMS: W0 D3 (T12 FPA 50k lr5e-4 **β_nl 1e-6**, H-48 primary, ~11.2h pole) · W1 D4 (T12 FPA 50k lr1e-3 β_nl 1e-6) · W2 D1 (T6 FPA 50k, ~6h → eval claims) · W3 D2 (C3-recipe s1 20k, ~2.4h → eval claims). **ONE-SHOT ARMS: a NaN auto-amputates in-chain (STOPPED label, evals on the final) — NO retries, NO lr surgery; this is registered science, not an incident.** Mumbai-first zones (asia-south1-c proven). Sentinel `CHAIN-SPORTBR2B-COMPLETE`.
**BUDGET CAPS (registered):** deadline = launch + 22h in `runs/tpu_deadline.txt` — **DO NOT extend without an explicit PI go** (B2; watchdog enforces; max node spend ≈ $300); canary projection gate (B1: projection > 1.2× the $270 top → hold after PHASE1 banking + PI framing); churn circuit-breaker (B3: ≥2 node losses in 2h during US-day hours → stop the supervisor, notify PI, wait for the night window unless PI says ride); chain FROZEN (B5: live verbs = kill-orphan / extend-deadline-with-PI-go / pause only); heartbeat carries the phase-wise spend curve (B6: PHASE1 ≈$165 by T+12h, evals ≈$220 by T+16h) + positive-describe after any down (session procedure). Expected COMPLETE ≈ T+13.5–16.5h; §6 close = counts + `audit_sportB_integrity.py --tag sportBr2b --phase final` 0-FAIL + ONE ledger line + ONE commit. Analysis = fresh session on PI go (`analyze_sportBr2b.py` untouched, self-test 20/20) → d128 registration per the carrier rules.

## ✅ COMPLETE — PHASE B RUNG 2 (2026-08-29 08:22:12Z chain-complete + self-teardown; C4_vb screen repaired 09:29Z on a one-shot node; FLEET ZERO verified 11 zones ×2; STRICT AUDIT 329/0/0 = `runs/audit_sportBr2_final_20260829.txt`; rung-1 repro 339/0/0). The 2026-08-29 ledger §5 entry is THE record (both carrier stops + labels, winner-caveat [C4_vb absent at the chain's argmax — analyzer revalidates from complete screens], incident→fix register, quarantine note, x-screens, spend ≈$500-510). Final data: `runs/` (extracted, canonical) + `gs://qhrrn2-rescue/sportBr2`. ANALYSIS = FRESH SESSION ON PI GO: `analyze_sportBr2.py` untouched (0-diff vs 3a8f18a, 20/20). RUNG-2B SKETCH below stands. All session layers/timers/crons STOPPED; launchd watchdog stands (deadline 17:16Z Aug 29 — nothing up).
...
## RUNG-2B SKETCH (PI-agreed 2026-08-29 ~01:15 IST: "we might need a rung 2b after the analysis") — FOR THE ANALYSIS SESSION TO FORMALIZE (rules pre-data): a compact stabilized deep-anchored cell at d96 — the carrier recipe (T12, FPA) with a REGISTERED stabilization chosen FROM the rung-2 forensics (banked monitor trajectories λ/η around both NaN onsets at 1e-3 AND 5e-4; candidates: lr 2.5e-4, lr-warmup, grad-clip-as-registered-recipe-change), ×2 seeds, ~$150-180, one night on this pod class, own breadth screen + optional 20k scan; registered as rung-2b standalone OR folded into the d128 registration. PHASE4-of-C3 does NOT block it — breadth scans are arm-local and a stabilized-carrier scan can run under its own registration any time.

## ▶ LIVE — BOTH CARRIER SEEDS STOPPED BY THE REGISTERED RULE (2026-08-28 ~09:15Z): C1 NaN'd AGAIN at half-lr (step 22,500) and C1s1 at 12,200 (both during the 07:45-08:55Z IPv6-blind window; the chain NaN guard HALTED each ≤5 min and kept the bank clean — markers PRETRAIN-{C1,C1s1}-NAN-HALTED in w0/w1 detached.log). ONE-retry-spent → both arms STOPPED, no lr iteration. AMPUTATION EXECUTED (ops, labeled): finals = last clean grids (C1_ckpt.pkl←20k, C1s1_ckpt.pkl←10k), truncated finite metrics, select_ckpt val_best (both vb==final), {arm}_STOPPED.txt labels, 4 zero-byte impossible-screens (m1/m2 > stopped step) banked; auditor extended (STOPPED-aware legit-empties + metrics tops; sportB repro still 339/0/0); w0/w1 relaunched → chains SKIP the stopped arms → evalcheaps on the finals → PHASE2 (claims cleared pre-relaunch; full_C3_t64 partials resume). SCIENCE READING (labeled, for the analysis pass): free FPA-T12 UNTRAINABLE at d96 at registered AND half lr under the resume regime while priced C2/C4 + T6 C3 train clean — the registered price-stabilizes contingency class, now on the carrier. ALSO: all four arms HBM-OOM'd into --remat retries today (numerics-equivalent by named test; NaN-remat correlation noted for the record). C2 ETA ~13:00Z, C4 (remat) ~15:30Z; PHASE2 running on freed chips; COMPLETE ≈20:00-23:30Z (fulls ×7 + PHASE4 + t256 depth rider are the tail) — EXTEND the 21:30Z deadline before it binds. AUTONOMOUS MODE (PI 13:10 IST re-grant) owns it to §6. RIDING the 05:17Z Mumbai v6e-16, all four arms clean (C1/C1s1 labeled half-lr PAST their old NaN zones; NaN guard + ec_one busy-retry live chain-wide; C3 done + 6/6 evals banked). COMPLETE ≈18:00-19:00Z → §6 (fleet-zero ELEVEN zones now — asia-south1-a/b/c joined the watchdog sweep after the morning coverage-hole fix; auditor --tag sportBr2 --phase final 0-FAIL; ONE ledger line carrying both days' incident register [C1+C1s1 LR-RETRY labels, eval-collision fix, teardown-verify defect, watchdog-zone lesson, orphan-gsutil lesson, ~\$30 meter-blind Mumbai hours] + ONE commit). Deadline 21:30Z. The night-pause block below is historical.

## ⏸ SUPERSEDED — NIGHT CLOSE 2026-08-27 20:07Z / 01:37 IST Aug 28 (PI: "stop the work for today... pick it back up in the morning"; autonomous mode SHELVED — PI wants to be available while errors are being worked through; re-grant explicitly on resume if desired). FLEET ZERO VERIFIED (10 zones nodes + QRs incl. all Mumbai zones); supervisor/caffeinate/04:57-timer/all monitors/all crons STOPPED; launchd watchdog STANDS (deadline 07:42Z Aug 28 — RESUME MUST EXTEND IT FIRST).
