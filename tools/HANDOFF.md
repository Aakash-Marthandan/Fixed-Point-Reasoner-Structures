...
## ▶ LIVE — CHAMPION TRACK PILOT (sportC0) launched 2026-09-01 — seven native9 d96 arms, one night

**Registration:** ledger §5 2026-09-01 "CHAMPION TRACK PILOT — LAUNCH REGISTRATION" (rules locked in `analyze_sportC0.py`, selftest 16/16; predictions + the law-interaction matrix are in the entry; `Plan_2026-09-01_Champion_Track.md` = the design). **Campaign:** `tools/campaign.env` ← campaign_sportC0.env (tag sportC0; US-first zones east1-d-first; v6e-16-first [4×4: W0 P1,P6 · W1 P2,P2s1 · W2 P3,P5 · W3 P3s1+riders] / v6e-8 fallback [1×8 sequential]; WALL 30600; sentinel CHAIN-SPORTC0-COMPLETE; final sportC0_final.tgz; SELF_TEARDOWN=1). **Budget:** $85–125 (16) / $80–110 (8), INFERRED until canary — B1 projection gate 1.2×; **deadline = launch+12h** (`runs/tpu_deadline.txt`; extension = PI go); node-side delete guard planted per node (the standing procedure); Mumbai = US-dry fallback ONLY. **One-shot arms:** NaN → in-chain amputation + STOPPED (stage-A death of a two-stage arm = STOP, no stage-B rescue — harness-verified). **Ops verbs allowed live:** kill-orphan / extend-deadline-with-PI-go / pause; chain FROZEN (B5). **Decision table:** PRETRAIN-NAN = registered outcome (amputation banked, chain continues) · EVAL-N-BAD / MERGE-N-BAD = STOP AND REPORT (partition integrity) · preemption → supervisor re-hunts, partials auto-resume (≤10 min loss), re-plant guard on the new node · INCOMPLETE = read why, never blind-relaunch. **HARD BOUNDARY: no accuracy reads during ops; `analyze_sportC0.py` untouched; the verdict + champion-d128 registration (sportC1) = the analysis pass on PI go.** On COMPLETE: verify sentinel + final tgz at source → self-teardown positive-describe → fleet-zero (8 zones nodes+QRs) → pull + extract → counts only → report.
