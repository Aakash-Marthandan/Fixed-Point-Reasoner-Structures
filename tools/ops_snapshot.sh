...
E=$(ls -td runs/sx*/ runs/sx*/*/ runs/filler_sx*/ runs/c8x_*/ runs/decarceval_*/*/ 2>/dev/null | grep -vE "/(records|partial|s[0-9])" | head -1)   # nested eval dirs (sxeval_p*/full_*, decarceval_<arm>/<set>) too
if [ -n "$E" ]; then
