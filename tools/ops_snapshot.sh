...
set -uo pipefail
cd "$(dirname "$0")/.." || exit 1
source "${POD_ENV:-tools/campaign.env}"   # POD_ENV (2026-09-16, the two-pod night): one snapshot per pod
...
E=$(ls -td runs/sx*/ runs/sx*/*/ runs/filler_sx*/ runs/c8x_*/ runs/c5l_*/ runs/decarceval_*/*/ 2>/dev/null | grep -vE "/(records|partial|s[0-9])" | head -1)   # nested eval dirs (sxeval_p*/full_*, decarceval_<arm>/<set>) too
if [ -n "$E" ]; then
